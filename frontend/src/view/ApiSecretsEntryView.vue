...
const message = computed(() => {
    ...
    if (!hasTenants.value) {
        return "当前没有可用 tenant，无法进入 API Secrets 页面。";
    }

    return "正在进入 API Secrets 页面...";
});

async function resolveApiSecretsEntry() {
    if (!authStore.isLoggedIn) {
        return;
    }
    ...
    <section class="flex flex-col gap-6">
        <header class="flex flex-col gap-2">
            ...
            <p class="max-w-2xl text-sm text-base-content/70">
                从最近使用的 tenant 恢复 API Secrets 视图。
            </p>
        </header>
