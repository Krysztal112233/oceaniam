...
///    it is used as-is.
/// 2. In debug builds (`kDebugMode`), use `http://localhost:8000` so local
///    `flutter run` hits the Axum service directly.
/// 3. Otherwise, on web, fall back to `${window.location.origin}/api` so the
///    same built artifact works behind any reverse-proxy / gateway.
/// 4. Outside web (or when JS interop is unavailable), use `http://localhost:8000`.
